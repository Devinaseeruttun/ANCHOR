...
def sortCollapsedColumn(df,referenceCol,colName):
    ...
    df = df.join(s) #join on index
    #df has one line per subject id. I'll sort the subject ids
...
def createAnnotationName(data):
    ...
    data.loc[(data['LastCommonTaxon'] == 1) & (data['AmbiguousHits'] > 1), ['taxon1','taxon2','taxon3','taxon4','taxon5','taxon6','taxon7']] = data.loc[(data['LastCommonTaxon'] == 1) & (data['AmbiguousHits'] > 1), 'taxon1'].map(str) + "_AS"
    data.loc[(data['LastCommonTaxon'] == 2) & (data['AmbiguousHits'] > 1), ['taxon2','taxon3','taxon4','taxon5','taxon6','taxon7']] = data.loc[(data['LastCommonTaxon'] == 2) & (data['AmbiguousHits'] > 1), 'taxon2'].map(str) + "_AS"
    data.loc[(data['LastCommonTaxon'] == 3) & (data['AmbiguousHits'] > 1), ['taxon3','taxon4','taxon5','taxon6','taxon7']] = data.loc[(data['LastCommonTaxon'] == 3) & (data['AmbiguousHits'] > 1), 'taxon3'].map(str) + "_AS"
    data.loc[(data['LastCommonTaxon'] == 4) & (data['AmbiguousHits'] > 1), ['taxon4','taxon5','taxon6','taxon7']] = data.loc[(data['LastCommonTaxon'] == 4) & (data['AmbiguousHits'] > 1), 'taxon4'].map(str) + "_AS"
    data.loc[(data['LastCommonTaxon'] == 5) & (data['AmbiguousHits'] > 1), ['taxon5','taxon6','taxon7']] = data.loc[(data['LastCommonTaxon'] == 5) & (data['AmbiguousHits'] > 1), 'taxon5'].map(str) + "_AS"
    data.loc[(data['LastCommonTaxon'] == 6) & (data['AmbiguousHits'] > 1), ['taxon6','taxon7']] = data.loc[(data['LastCommonTaxon'] == 6) & (data['AmbiguousHits'] > 1), 'taxon6'].map(str) + "_AS"
    #data.loc[(data['LastCommonTaxon'] == 7) & (data['AmbiguousHits'] > 1), 'taxon7'] = data.loc[(data['LastCommonTaxon'] == 7) & (data['AmbiguousHits'] > 1), 'taxon7'].map(str) + "_AS"
    ...
    #Bacteria;Firmicutes;Clostridia;Clostridiales;Clostridiales;Eubacterium;Eubacterium    will produce    Bacteria;Firmicutes;Clostridia;Clostridiales;Clostridiales;Clostridiales_AS;Clostridiales_AS
    #while 
    #Bacteria;Firmicutes;Clostridia;Clostridiales;Lachnospiraceae;Lachnospiraceae;Lachnospiraceae    will produce    Bacteria;Firmicutes;Clostridia;Clostridiales;Clostridiales_AS;Clostridiales_AS;Clostridiales_AS
    #They'll both have the same _AS name although their laast taxonomic level will be different.
    #Solution: I will lower the taxonomic level of the first so it will be identical to the last one
    # 1. Crete fake taxons (the same as the other ones, but without _AS)
    for i in range (1,8,1):
        data["Faketaxon%i"%i] = [x.rsplit("_AS",1)[0] for x in data['taxon%i'%i]]
    ...
    pattern = ';'
    data.loc[(data['FakeCommonLastTax'] == 1) & (data['AmbiguousHits'] > 1), ['taxon1','taxon2','taxon3','taxon4','taxon5','taxon6','taxon7']] = data.loc[(data['FakeCommonLastTax'] == 1) & (data['AmbiguousHits'] > 1), 'Faketaxon1'].map(str) + "_AS"
    data.loc[(data['FakeCommonLastTax'] == 2) & (data['AmbiguousHits'] > 1), ['taxon3','taxon4','taxon5','taxon6','taxon7']] = data.loc[(data['FakeCommonLastTax'] == 2) & (data['AmbiguousHits'] > 1), 'Faketaxon2'].map(str) + "_AS"
    data.loc[(data['FakeCommonLastTax'] == 3) & (data['AmbiguousHits'] > 1), ['taxon4','taxon5','taxon6','taxon7']] = data.loc[(data['FakeCommonLastTax'] == 3) & (data['AmbiguousHits'] > 1), 'Faketaxon3'].map(str) + "_AS"
    data.loc[(data['FakeCommonLastTax'] == 4) & (data['AmbiguousHits'] > 1), ['taxon5','taxon6','taxon7']] = data.loc[(data['FakeCommonLastTax'] == 4) & (data['AmbiguousHits'] > 1), 'Faketaxon4'].map(str) + "_AS"
    data.loc[(data['FakeCommonLastTax'] == 5) & (data['AmbiguousHits'] > 1), ['taxon6','taxon7']] = data.loc[(data['FakeCommonLastTax'] == 5) & (data['AmbiguousHits'] > 1), 'Faketaxon5'].map(str) + "_AS"
    data.loc[(data['FakeCommonLastTax'] == 6) & (data['AmbiguousHits'] > 1), ['taxon7']] = data.loc[(data['FakeCommonLastTax'] == 6) & (data['AmbiguousHits'] > 1), 'Faketaxon6'].map(str) + "_AS"
    data.loc[data['AmbiguousHits'] > 1, 'taxonomy'] = data['taxon1'] + ";" + data['taxon2'] + ";" + data['taxon3'] + ";" + data['taxon4'] + ";" + data['taxon5'] + ";" + data['taxon6'] + ";" + data['taxon7']
...
parser.add_argument('-p', '--primers', help='[OPTIONAL] Collapse anchors based on primer ambiguity nucleotides if exist. It will use the anchor fasta file produced in part5 (anchors_fasta.tsv) and the primer list used in part2 (metadata/primers_parsed.txt)', dest='primers', default=False, action='store_true', required=False)
...
if args.primers == True:
    try:
        primers = pd.read_csv("primers_parsed.txt", sep='\t')
    except:
        print "\n---\nYou need a copy or link of primers_parsed.txt (check Metadata folder) in this folder"
        sys.exit(1)
...
if args.primers == True:
    ...
    for i in range (0,len(primers)):
        ...
        #print "REVERSE primer (rev complement): ", reverse2
        #fasta.to_csv("FASTA1",sep="\t")
        ...
        #print  "len after:",len(fasta)
        #fasta.to_csv("FASTA2",sep="\t")
    ...
    print "Original data length: ", checkDataLen
    print "New data length: ", len(data_tempFinal)
    try:
        checkDataLen == len(data_tempCheck)
        ...
        print "Original data size: ", len(checkDataLen)
...
if output_option == "SubOTU":
    ...
    print("TEST1: ",len(data.loc[data["subjectid"]=='silva|DQ925472.1.1531',"subjectid"]))
    data = sortCollapsedColumn(data, "queryid", "subjectid")
    print("TEST2: ",len(data.loc[data["subjectid"]=='silva|DQ925472.1.1531',"subjectid"]))
    data.to_csv("DATA3", sep="\t", index=False)
    ...
    data.to_csv("DATA4", sep="\t", index=False)
    ...
    data.to_csv("DATA5", sep="\t", index=False)
