...
def collapseAndSort(mainDF,colName):
    secondaryAmbiSubOTU = mainDF.groupby("subOTU").agg({'%s'%colName:RowCollapse})
    ...
    mainDF = mainDF.drop(['%s'%colName],1)
    mainDF = mainDF.merge(secondaryAmbiSubOTU, on=['subOTU'], how='inner')
    return mainDF
